// icon:movie | Material Design Icons https://materialdesignicons.com/ | Austin Andrews

import React, { useImperativeHandle, forwardRef } from "react";

export interface MovieIconRef {
  getIconName: () => string;
  getIconType: () => string;
  getViewBox: () => string;
  getDimensions: () => { width: string; height: string };
}

const MovieIcon = forwardRef<MovieIconRef, {}>(({}, ref) => {
  useImperativeHandle(
    ref,
    () => ({
      getIconName: () => "MovieIcon",
      getIconType: () => "movie",
      getViewBox: () => "0 0 24 24",
      getDimensions: () => ({ width: "1em", height: "1em" }),
    }),
    []
  );
  return (
    <svg viewBox="0 0 24 24" fill="currentColor" height="1em" width="1em">
      <path d="M18 4l2 4h-3l-2-4h-2l2 4h-3l-2-4H8l2 4H7L5 4H4a2 2 0 00-2 2v12a2 2 0 002 2h16a2 2 0 002-2V4h-4z" />
    </svg>
  );
});

MovieIcon.displayName = "MovieIcon";
export default MovieIcon;
